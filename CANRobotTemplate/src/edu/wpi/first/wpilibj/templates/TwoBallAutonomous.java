...
import edu.wpi.first.wpilibj.templates.RobotTemplate;//this is for Pickup() with autoChooser for choosing autonomous
import edu.wpi.first.wpilibj.SimpleRobot;
 ...
 * @author gsesny
 */
public class TwoBallAutonomous extends Command{
  ...
  protected void initialize() {
    ...
    public void robotInit(){
        ...
        SmartDashboard.putNumber("long_BeltLength", -8.6); //the two doubles that are written in the following line are default values and can be changed. Change the default values to the starting values of the robot in autonamous. Devin
        ...
        SmartDashboard.putNumber("mediumShort_BeltLength", -10.0);//the two doubles that are written in the following line are default values and can be changed. Change the default values to the starting values of the robot in autonamous. Devin
        SmartDashboard.putNumber("short_BeltLength", -2.1);//the two doubles that are written in the following line are default values and can be changed. Change the default values to the starting values of the robot in autonamous. Devin
...
//        autoChooser.addDefault("Default Program", new OneBallAutonomous());
//        autoChooser.
    }
...
//        compressor.start();
        ...
        winchEncoder.reset();  //Used to make sure encoder is set to 0 at the start
                ...
                while(winchEncoder.getDistance() > -11.0){ //Unwind winch to belt length necessary to shot from starting position
                    winch1.set(1.0);
                    winch2.set(1.0);
        ...
        while(winchEncoder.getDistance() > -5.0){ //Unwind winch to belt length necessary to shot from starting position
                    winch1.set(1.0);
                    winch2.set(1.0);
...
//        
//        
//                while(winchEncoder.getDistance() > -11.6){ //Unwind winch to belt length necessary to shot from starting position
//                    winch1.set(1.0);
//                    winch2.set(1.0);
//                }
//                winch1.set(0.0);
...
//            VisionCounter++;
            ...
            if (controller.getRawButton(button_Start)) {
                ...
                releaseWinch(SmartDashboard.getNumber("long_BeltLength", -8.6));
            }      
            if(controller.getRawButton(button_X)){
                releaseWinch(SmartDashboard.getNumber("mediumShort_BeltLength", -10.0));
            }
            if(controller.getRawButton(button_B)){
                releaseWinch(SmartDashboard.getNumber("short_BeltLength", -2.1));
            }
            if(controller.getRawButton(button_Y)){
                releaseWinch(SmartDashboard.getNumber("mediumLong_BeltLength", -6.0));
            }
